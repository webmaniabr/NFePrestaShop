...
class WebmaniaBrNFe extends Module{
  ...
  public function getConfigFieldsValues(){
    return array(
      ...
      $this->name.'mask_doc_fields' => Configuration::get($this->name.'mask_doc_fields'),
      $this->name.'auto_address_fill' => Configuration::get($this->name.'auto_address_fill'),
    );
  }
  ...
  function getConfigInitValues(){
    return array(
      ...
      $this->name.'mask_doc_fields' => 'on',
      $this->name.'auto_address_fill' => 'on',
    );
  }
  ...
  ***********************************************************/
  ...
  public function hookActionObjectAddressUpdateBefore($params){
    ...
    if(Tools::isSubmit('submitAddress')){
      ...
      $address_number = Tools::getValue('address_number');
      $address_county = Tools::getValue('address2');
      $update_values = array();

      if( !$address_number || !$address_county ){
        //In case user disable pass JS validation without valid data, redirect so the address doesnt get updated
        Tools::redirect(__PS_BASE_URI__);
      }else{
        $update_values['nfe_number'] = pSQL($address_number);
      }

      if(Tools::isSubmit('person_type')){
        $person_type = Tools::getValue('person_type');
        $cpf = Tools::getValue('cpfValue');
        $cnpj = Tools::getValue('cnpjValue');
        $inscricao_estadual = Tools::getValue('ieValue');
        if( ($person_type == 1 && !$cpf) || ($person_type == 2 && !$cnpj) || ($person_type != 1 && $person_type != 2) || ($person_type == 1 && !$this->validaCPF($cpf)) || ($person_type == 2 && !$this->validaCNPJ($cnpj)) || ($person_type == 2 && !$inscricao_estadual) ){
          //In case user disable pass JS validation without valid data, redirect so the address doesnt get updated
          Tools::redirect(__PS_BASE_URI__);
        }else{
          $update_values['nfe_tipo_pessoa'] = pSQL($person_type);
          if($person_type == 1){
            $update_values['nfe_cpf'] = pSQL($cpf);
          }else if($person_type == 2){
            $update_values['nfe_cnpj'] = pSQL($cnpj);
            $update_values['nfe_ie'] = pSQL($inscricao_estadual);
          }
        }
      }

      if(!Db::getInstance()->update('address', $update_values, 'id_address = ' .$address_id)){
        Tools::redirect(__PS_BASE_URI__);
      }
    }
  }
  ...
  public function hookActionObjectAddressAddAfter($params){
    if(Tools::isSubmit('submitAddress')){
      ...
      if( !$address_number || !$address_county ){
        //In case user pass JS validation without valid data, redirect so the address doesnt get added
        Tools::redirect(__PS_BASE_URI__);
      }else{
        $update_values['nfe_number'] = pSQL($address_number);
      }
      ...
      if(Tools::isSubmit('person_type')){
        ...
        $cnpj = Tools::getValue('cnpjValue');
        $inscricao_estadual = Tools::getValue('ieValue');
        if( ($person_type == 1 && !$cpf) || ($person_type == 2 && !$cnpj) || ($person_type != 1 && $person_type != 2) || ($person_type == 1 && !$this->validaCPF($cpf)) || ($person_type == 2 && !$this->validaCNPJ($cnpj))){
          //In case user disable JS validation without valid data, redirect so the address doesnt get added
          Tools::redirect(__PS_BASE_URI__);
        }else{
          $update_values['nfe_tipo_pessoa'] = pSQL($person_type);
          if($person_type == 1){
  ...
  public function hookDisplayHeader($params){
    ...
    if(_MAIN_PS_VERSION_ == '1.5'){
      ...
      if($controller == 'AddressController' || $controller == 'OrderController' || $controller == 'AuthController' || $controller == 'WebmaniaBRNFeViewModuleFrontController'){
    ...
    if(_MAIN_PS_VERSION_ == '1.6'){
      ...
      if(Configuration::get($this->name.'person_type_fields') == 'on'){
        Media::addJsDef(array('add_person_fields' => true));
      }

      if(Configuration::get($this->name.'mask_doc_fields') == 'on'){
        Media::addJsDef(array('mask_doc_fields' => true));
      }

      if(Configuration::get($this->name.'auto_address_fill') == 'on'){
        Media::addJsDef(array('auto_address_fill' => true));
      }
    }
  ...
  ***********************************************************/
  ...
  public function getOrderData($orderID){
    ...
    $data = array(
     ...
     if ($tipo_pessoa == 'cpf'){
         $data['cliente'] = array(
             'cpf' => $customer_custom['nfe_document_number'], // (pessoa fisica) Número do CPF
       ...
       $data['cliente'] = array(
         'cnpj' => $customer_custom['nfe_document_number'], // (pessoa jurídica) Número do CNPJ
